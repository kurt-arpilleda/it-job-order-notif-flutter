...
void main() async {
  ...
  const platform = MethodChannel('com.example.countdown_timer/volume');
  try {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'IT JOB ORDER NOTIF',
      theme: ThemeData(primarySwatch: Colors.blue),
      initialRoute: initialRoute,
      routes: {
        '/phorjapan': (context) => PhOrJpScreen(),
        '/webView': (context) => SoftwareWebViewScreen(linkID: 28),
        '/webViewJP': (context) => SoftwareWebViewScreenJP(linkID: 28),
      },
    );
  }
}